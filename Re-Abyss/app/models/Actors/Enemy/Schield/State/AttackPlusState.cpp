...
namespace abyss::Schield
{
    ...
    void AttackPlusState::draw() const
    {
        if (m_transitionToAttackCross.isRunning()) {
            m_pActor->getBindedView()->drawToAttackCross(m_transitionToAttackCross.progress0_1());
        } else {
            m_pActor->getBindedView()->drawToAttackPlus(m_transitionToAttackPlus.progress0_1());
        }
    }
}
