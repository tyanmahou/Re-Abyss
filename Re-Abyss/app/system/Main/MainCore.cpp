...
{
    MainCore::MainCore(Cycle::Main::IMasterObserver* pObserver):
    ...
    void MainCore::init(const std::shared_ptr<Actor::ActorObj>&player)
    {
    ...
    void MainCore::loadStage(const std::shared_ptr<StageData>& stageData)
    {
        m_stage->setStageData(stageData);
        m_stage->load();
    }
    void MainCore::loadTemporaryData(const std::shared_ptr<TemporaryData>&tempData)
    {
        m_temporary->setTemporaryData(tempData);
    }
    std::shared_ptr<Actor::ActorObj> MainCore::lockPlayer() const
    {
