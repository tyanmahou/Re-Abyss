...
namespace abyss::UI::SaveSelect::Main
{
    ...
    void ModeCtrl::onDraw() const
    {
        if (m_mode == Mode::GameStart) {
            FontAsset(FontName::SceneName)(U"- {} -"_fmt(Msg::SaveSelect::Mode::GameStart)).drawAt(PivotUtil::FromTc(0, 50), Color(0, 255, 255));
        } else {
            FontAsset(FontName::SceneName)(U"- {} -"_fmt(Msg::SaveSelect::Mode::Delete)).drawAt(PivotUtil::FromTc(0, 50), Color(255, 0, 0));
        }
    }
}
