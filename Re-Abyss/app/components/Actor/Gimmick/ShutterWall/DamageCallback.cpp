...
namespace
{
    ...
    TempKey ShutterWallBreakKey(ActorObj* pActor, s3d::uint32 id)
    {
        return TempKey::ShutterWallBreak(pActor->getModule<Stage>()->mapName(), id);
    }
}
namespace abyss::Actor::Gimmick::ShutterWall
{
    ...
    {
        // 取得済みなら破棄
        if (m_pActor->getModule<Temporary>()->onFlag(::ShutterWallBreakKey(m_pActor, 0))) {
            m_pActor->destroy();
            return;
        }
    }
    ...
    void DamageCallback::onDead()
    {
        ...
        // フラグ保存
        m_pActor->getModule<Temporary>()->saveFlagRestart(::ShutterWallBreakKey(m_pActor, 0));
    }
}
